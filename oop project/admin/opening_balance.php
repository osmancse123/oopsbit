...
if (isset($_POST["addbtn"])) 
  {
    if (!empty($date) && !empty($Year) && !empty($Hand_in_Cash) && !empty($Bank_Deposit) && !empty($Account_Payable)  && !empty($Account_Receiveable) && !empty($Admin)) 
      {
         $insert="INSERT INTO `openingbalance` (`date`,`year`,`hend_in_cash`,`Bank_Deposit`,`accounts_receiveable`,`accounts_payable`,`admin`) VALUES ('$date','$Year','$Hand_in_Cash','$Bank_Deposit','$Account_Receiveable','$Account_Payable','$Admin')";
         $ob->insert($insert);
      }
  }
  ...
  if(isset($_GET["editbtn"]))
    {
        ...
        $sql="SELECT * FROM `openingbalance` WHERE `id`='".$_GET["editbtn"]."'";
        $r=$ob->select($sql);
          if($r)
            {
      ...
      if (isset($_POST["editbtn"])) 
          {
            if (!empty($date) && !empty($Year) && !empty($Hand_in_Cash) && !empty($Bank_Deposit) && !empty($Account_Payable)  && !empty($Account_Receiveable) && !empty($Admin)) 
              {
                 $edit="REPLACE INTO `openingbalance` (`id`,`date`,`year`,`hend_in_cash`,`Bank_Deposit`,`accounts_receiveable`,`accounts_payable`,`admin`) VALUES ('".$_GET["editbtn"]."','$date','$Year','$Hand_in_Cash','$Bank_Deposit','$Account_Receiveable','$Account_Payable','$Admin')";
                 $ob->edit($edit);
              }
          }
    ...
    if(isset($_GET["delbtn"]))
      {
        $del="DELETE FROM `openingbalance` WHERE id='".$_GET["delbtn"]."'";
        $ob->del($del);
      }
    ...
    <form method="POST">
      ...
      ?>
        <table class="table table-bordered">
          ...
          <?php
            $sql="select * from `openingbalance`";
