<?php
    ...
    global $chek_query;
	$prefix=date("y"."m"."d");
$fetch_query[0]=$db->withoutPrefix('meritorious_student','student_id',"16".$prefix,'12');
...
//                    print "asdf";
                ...
                @$inserquery="INSERT INTO `meritorious_student`(`student_id`,`student_name`,`session`,`achivement`) VALUES('".$fetch_query[0]."','$name','$session','$detaile')";
...
if(isset($_POST["view"]))
{
    $query="select * from meritorious_student ORDER BY `student_id` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='Successfull_student.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>Piece students Information</span>"."</td>"."</tr>";
        if($result){
            $table.="<tr>"."<td>Name</td>"."<td>Session</td>"."<td> Achivement</td>"."<td>File</td>"."<td>Edit Or Delete</td>"."</tr>";
                ...
                while($fetch_All=$result->fetch_array())
                {
                   ...
                   $table.="<tr>";
                    for ($i=1; $i < $count; $i++) { 
                        ...
                        $table.="<td>"."<a href=\"../other_img/".$fetch_All[0].".jpg\" target='_blank' class='btn btn-primary'>Download</a>"."</td>";
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `meritorious_student` WHERE `student_id`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='Successfull_student.php'</script>";
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM meritorious_student WHERE `student_id`='".$_GET["edit"]."'";
        ...
        $expensions= array("jpeg","jpg","png","pdf","","txt");         
                ...
                if(in_array($file_ext,$expensions) === false){
                 ...
                 $msg='File size must be excately 2 MB';
                    ...
                    //print "asdf";
               @$inserquery="Replace INTO `meritorious_student`(`student_id`,`student_name`,`session`,`achivement`) VALUES('".$_POST["id"]."','$name','$session','$detaile')";
...
<link rel="stylesheet" href="../css/loading/loading.css" />
    ...
    <?php if(isset($_POST["view"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Record  Found"."<a href='Successfull_student.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
...
Piece students Information</span> </td>
            </tr>
         <tr class="info">
        ...
        <td>
          ...
          <input type="text"  name="name" <?php if($chek_query){?>  value="<?php echo $fetch_query[1];?>" <?php } else {?> value="" <?php }?> class="form-control" placeholder="Name" />
          </div>
          </td>
      </tr>
      ...
      <tr class="info">
        ...
        <td>
          ...
          <input type="text" name="session" <?php if($chek_query){?> value="<?php  echo $fetch_query[2];?>" <?php } else {?> value="" <?php }?> class="form-control" placeholder="2013-2014" />
          </div>
          </td>
      </tr>
    ...
    <tr class="info">
        <td align="right">  
Details For Acchived &nbsp;</td>
        <td>:</td>
         <td>
            <div class="col-sm-12 col-md-12">
            <textarea  name="descreption" class="form-control"  id="redactor"><?php if($chek_query){echo $fetch_query[3]; } else { echo ""; }?> </textarea>
            </div>
            </td>
