<?php
    ...
    global $chek_all;
	$prefix=date("y"."m"."d");
    $id=$db->withoutPrefix('admission_from','id',"17".$prefix,'12');
 ...
 if($chek_all)
 {
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf");         
             ...
             @$inserquery="INSERT INTO `admission_from` VALUES('$id','$title','$file_ext')";
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `admission_from` WHERE `id`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='Addmission_form.php'</script>";
    $id=$db->withoutPrefix('admission_from','id',"17".$prefix,'12');
...
if(isset($_POST["View"]))
{
    $query="select * from admission_from ORDER BY id DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='Addmission_form.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>Admission Form</span>"."</td>"."</tr>";
        if($result){
            $table.="<tr>"."<td>Title</td>"."<td> File</td>"."<td> Delete</td>"."</tr>";
            $count=mysqli_num_fields($result);
                while($fetch_All=$result->fetch_array())
                {
                   $table.="<tr>";
                    ...
                    $table.="<td>".'<a href="../other_img/'.$fetch_All[0].'.'.$fetch_All[2].'" class="btn btn-primary" target="_blank">Download</a>'."</td>";
                   
                        $table.="<td>";
                         
                        $table.="<a style='width:80px; margin-left:5px; margin-top:2px;' href='?dlt=$fetch_All[0]&ext=$fetch_All[2]' class='btn btn-danger' onclick='return confirm_delete()'>Delete</a>";
                        $table.="</td>";
                 $table.="</tr>";
  ...
  <head>
   ...
   <link rel="stylesheet" href="../css/loading/loading.css" />
    ...
    <?php if(isset($_POST["View"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Addmission Form  Found"."<a href='Addmission_form.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
