...
WHERE `result`.`classId`='$clId' AND `result`.`GroupID`='$gpId' AND `result`.`session`='$Session'  GROUP BY  `result`.`STD_ID` ORDER BY `result`.`std_roll` ASC LIMIT $limit1,$limit2";
  ...
  if($result){
      ...
      $DELETE="DELETE FROM `gnerate_marks` WHERE `studentID`='$fetch_r[STD_ID]' AND  `ExamID`='$fetchFinalExamType[0]'";
...
if($totalSubjectpartObtainMarks>=$totalsubpartmarks*80/100 &&  $totalSubjectpartObtainMarks<=$totalsubpartmarks*100/100)
  ...
  else
  {
     ...
     $CGPA="0.00";
  		...
  		$insertQuery="INSERT INTO `gnerate_marks`(`studentID`,`ClassID`,`GroupID`,`ExamID`,`session`,`studentRoll`,`subjectID`,`fullMarks`,`obtainMarks`,`letterGrade`,`gradePoint`)VALUES('$fetch_r[STD_ID]','$clId','$gpId','$fetchFinalExamType[0]',
        '$Session','$studentRoll','$fetchsubject[0]','$fetchMarks[total]','$totalMarks','$grade','$CGPA')";
...
//print $fetchMarks['subject_code'];
 $totalComMarks= $helfYearly+$totrialAvg;
 //print  $helfYearly.$fetchMarks['total']."<br>";
...
// `Year`
    ...
    // $insertResult="INSERT INTO `result`(`STD_ID`,`std_roll`,`classId`,`GroupID`,`session`,`examId`,`CGPA`,`witoutOptional`,`totalFailSub`,`Year`) VALUES('$fetch_r[STD_ID]','$studentRoll','$clId','$gpId','$Session','$fetchFinalExamType[0]','$cgpa','$cgpaWithoutOptional','','".date('Y')."')";
