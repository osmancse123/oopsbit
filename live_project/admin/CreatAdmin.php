...
<?php
	...
	if(isset($_POST["save"])){
		...
		if(isset($_POST["update"])){
					...
					$sql = 'DELETE FROM `sublinkpeority` WHERE `AdminId`="'.$_POST['id'].'"';
					$db->delete_query($sql);
					$sql2 = 'DELETE FROM `main_link_piority` WHERE `adminId`="'.$_POST['id'].'"';
		...
		if(isset($_POST["name"]) && !empty($_POST["name"]) && isset($_POST["Email"]) && !empty($_POST["Email"]) && isset($_POST["admintype"]) && !empty($_POST["admintype"]) && isset($_POST["Passward"]) && !empty($_POST["Passward"]) && isset($_POST["RePassward"]) && !empty($_POST["RePassward"])&& isset($_POST["status"]) && !empty($_POST["status"]) ){
			...
			if($_POST["Passward"] === $_POST["RePassward"]){
				...
				$coutsublink=count($_POST["SublinkID"]);
					...
					$insertsql="Replace INTO `admin_users` (`id`,`Name`,`email`,`status`,`type`,`pass`,`repass`) VALUES('$_POST[id]','$_POST[name]','$_POST[Email]','$_POST[status]','$_POST[admintype]','$pass','$Repass')";
					$chek=$db->insert_query($insertsql);
					$Paht="../other_img/".$_POST['id'].".jpg";
					move_uploaded_file($_FILES["file"]["tmp_name"],$Paht);
					
					for($x=0; $x<$countmainLink;$x++){
						$minInsert="INSERT INTO `main_link_piority` (`adminId`,`Main_Link_id`) VALUES ('$_POST[id]','".$_POST["linkID"][$x]."')";
						$resultmain=$db->insert_query($minInsert);
					}
					
					for($z=0; $z<$coutsublink; $z++){
							$explodevale=explode('and',$_POST["SublinkID"][$z]);
							$insertsublink="INSERT INTO `sublinkpeority` (`AdminId`,`MainLinkID`,`sublinkId`) VALUES ('$_POST[id]','$explodevale[0]','$explodevale[1]')";
							$resultInsertlink=$db->insert_query($insertsublink);
					}
...
<html lang="en">
 ...
 <script>
			...
			function editAdmin(getid){
						var editadmin ="ddd"; 
					$.ajax({
							url:"ajaxForAdmin.php",
							type:"POST",
							data : {getid:getid,editadmin:editadmin},
							 success : function(text)
							{
									$("#load-data-here").html(text);
								}
