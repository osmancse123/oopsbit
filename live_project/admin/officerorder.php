<?php
    ...
    global $chek_query;
	$prefix=date("m");
	$fetch_query[0]=$db->withoutPrefix('office_order','Notice_id',"3".$prefix,'10');
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","docx","doc","");         
                ...
                @$inserquery="INSERT INTO `office_order`(`Notice_id`,`Date_time`,`Title`,`Extension`,`Description`) VALUES('".$fetch_query[0]."','$date','$title','$file_ext','$detaile')";
...
if(isset($_POST["view"]))
{
    $query="select * from office_order ORDER BY `Date_time` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='officerorder.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>View Office Order</span>"."</td>"."</tr>";
        if($result){
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `office_order` WHERE `Notice_id`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='officerorder.php'</script>";
    $fetch_query[0]=$db->withoutPrefix('office_order','Notice_id',"3".$prefix,'10');
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM `office_order` WHERE `Notice_id`='".$_GET["edit"]."'";
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="REPLACE INTO `office_order`(`Notice_id`,`Date_time`,`Title`,`Extension`,`Description`) VALUES('".$_POST["id"]."','$date','$title','$file_ext','$detaile')";
				...
				//print "dd";
					...
					$selext = "SELECT `Extension` FROM `office_order` WHERE `Notice_id`='".$_POST["id"]."'";
					$resultext =  $db->select_query($selext);
					if($resultext > 0){
						$fetch_ext = $resultext->fetch_array();
					}
					
                                    @$inserquery="REPLACE INTO `office_order`(`Notice_id`,`Date_time`,`Title`,`Extension`,`Description`) VALUES('".$_POST["id"]."','$date','$title','$fetch_ext[0]','$detaile')";
                  
                   
                }  
                  @$chek_insert=$db->update_query($inserquery);
				   $select_query="SELECT * FROM `office_order` WHERE `Notice_id`='".$_POST["id"]."'";
        $chek_query=$db->select_query($select_query);
        if($chek_query){
            $fetch_query=$chek_query->fetch_array();
        }
...
<html lang="en">
 ...
 function checkforText()
    ...
    <?php if(isset($_POST["view"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Notice  Found"."<a href='officerorder.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
