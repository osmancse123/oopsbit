...
<html lang="en">
		...
		function enable_cb() {
		  if (this.checked) {
			...
			function AddPresent(){
				...
				$.ajax({
							url:"ajaxForStruffattendance.php",
							type:"POST",
  ...
  <body>
	 ...
	 <div class="col-lg-12">
              ...
              <tr>
                <td colspan="8" align="center">&nbsp;<span class="text-success" style="font-size:18px;"><strong>Struff attendance</strong></span></td>
              </tr>
              <tr>
			...
			<select name="olddate" id="olddate"  style="width:180px; height:28px;">
			  ...
			  <?php
			  if(!isset($_POST["submit"])){
			  		$sql="SELECT * FROM `teachers_information` WHERE `Type`='Stuff'  ORDER BY `index_no` ASC";
			  ...
			  <?php } } }
			  	if(isset($_POST["submit"])){ 
					if($_POST["olddate"] != "Select One..."){
					 ...
					 $sql5="SELECT `struff_present`.*,`teachers_information`.`teachers_name`,`designation` FROM `struff_present`
...
WHERE `struff_present`.`date`='".$_POST["olddate"]."' ORDER BY `struff_present`.`slNo` ASC";
				 	...
				 	/*$sql5="SELECT `struff_present`.*,`stuff_information`.`stuff_name`,`designation` FROM `struff_present`
INNER JOIN `stuff_information` ON `stuff_information`.`teachers_id`=`struff_present`.`StruffID` WHERE `struff_present`.`date`='".$_POST["olddate"]."'
ORDER BY `struff_present`.`slNo` ASC";*/
			 ...
			 <tr>
						...
						<td align="center"><?php echo $fetchSql["teachers_name"];?></td>
						<td  align="center"><?php echo $fetchSql["designation"];?></td>
						<td align="center"><input class="check_elmnt" type="checkbox" onClick="return singleChek('<?php echo  $fetchSql["StruffID"];?>')" name="linkID[]" value="<?php echo  $fetchSql["StruffID"]?>" id="snchek-<?php echo $fetchSql["StruffID"]?>" <?php if($fetchSql["present"]=='1'){?> checked="checked" <?php } else { ?> disabled="disabled" <?php } ?>/></td>
						<td align="center">
						
						<input type="text" name="attnTime[]" value="<?php echo $fetchSql["comming_time"]?>" id="attnTime-<?php echo $fetchSql["StruffID"]?>"  style="width:90px; text-align:center" readonly=""/></td>
						<td align="center"><input type="text" value="<?php  echo date('h'.':'.'m');?>"  name="leavTime[]" id="leavTime-<?php echo $fetchSql["StruffID"]?>"  style="width:90px; text-align:center"  /></td>
						<td align="center"><input <?php if($fetchSql["onvacation"]=='1'){?> checked="checked" <?php } else { ?> disabled="disabled" <?php } ?>   type="checkbox" class="approved-<?php echo $fetchSql["StruffID"]?>" id="group" name="approved[]" value="<?php echo $fetchSql["StruffID"];?>" onClick="return approvedFun('<?php echo $fetchSql["StruffID"];?>')" /></td>
						<td align="center"><input   <?php if($fetchSql["absent"]=='1'){?> checked="checked" <?php } else { ?> disabled="disabled" <?php } ?> type="checkbox" class="Unapproved-<?php echo $fetchSql["StruffID"]?>" id="group" name="unapproved[]" value="<?php echo $fetchSql["StruffID"];?>" onClick="return UNapprovedFun('<?php echo $fetchSql["StruffID"];?>')" /></td>
				</tr>
