<?php
    ...
    global $chek_query;
	$prefix=date("y"."m"."d");
$fetch_query[0]=$db->withoutPrefix('acchivement','ID',"11".$prefix,'12');
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","","txt");         
                ...
                @$inserquery="INSERT INTO `acchivement`(`ID`,`Title`,`Description`) VALUES('".$fetch_query[0]."','$title','$detaile')";
...
if(isset($_POST["view"]))
{
    $query="select * from acchivement ORDER BY `ID` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='Achivement.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='4' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>Achivement</span>"."</td>"."</tr>";
        if($result){
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `acchivement` WHERE `ID`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='Achivement.php'</script>";
$fetch_query[0]=$db->withoutPrefix('acchivement','ID',"11".$prefix,'12');
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM acchivement WHERE `ID`='".$_GET["edit"]."'";
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","","txt");         
                ...
                @$inserquery="REPLACE INTO `acchivement`(`ID`,`Title`,`Description`) VALUES('".$_POST["id"]."','$title','$detaile')";
                  ...
                  @$chek_insert=$db->update_query($inserquery);
				    $select_query="SELECT * FROM acchivement WHERE `ID`='".$_POST["id"]."'";
        $chek_query=$db->select_query($select_query);
        if($chek_query){
            $fetch_query=$chek_query->fetch_array();
        }
  ...
  <head>
   ...
   <link rel="stylesheet" href="../css/loading/loading.css" />
    ...
    <?php if(isset($_POST["view"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Recorde Found"."<a href='Achivement.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
