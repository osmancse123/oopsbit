<?php
    ...
    $select_max="SELECT MAX(`serial`) FROM `public_exam_result`";
    $chekc_query=$db->select_query($select_max);
    if($chekc_query)
    {
        $fetch_max=$chekc_query->fetch_array();
        $inc_max=$fetch_max[0]+1;
    }
        ...
        $expensions= array("jpeg","jpg","png","pdf","","txt");         
                ...
                if(in_array($file_ext,$expensions) === false){
                 ...
                 $msg='File size must be excately 2 MB';
                    ...
                    //print "asdf";
                @$inserquery="INSERT INTO `public_exam_result` (`serial`,`date`,`result_title`,`result`,`extension`) VALUES('$inc_max','$date','$title','$detaile','$file_ext')";
                  
                     @move_uploaded_file($file_tmp,"../other_img/"."$inc_max".".$file_ext");
                    ...
                    //print "adsf";
                                    ...
                                    @$inserquery="INSERT INTO `public_exam_result`(`serial`,`date`,`result_title`,`result`) VALUES('$inc_max','$date','$title','$detaile')";
                   
                }  
                  @$chek_insert=$db->insert_query($inserquery);
                    //print_r($inserquery);
                  //$fetch_query[0]=$db->autogenerat('acchivement','ID','ACCHIV-','12');
...
if(isset($_POST["view"]))
{
    $query="select * from public_exam_result ORDER BY `serial` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='public_exam_result.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>The test results of various public services</span>"."</td>"."</tr>";
        if($result){
            $table.="<tr>"."<td>Serial</td>"."<td>Date</td>"."<td>Title</td>"."<td> Result</td>"."<td> File</td>"."<td>Edit Or Delete</td>"."</tr>";
            ...
            //print_r($count);
                while($fetch_All=$result->fetch_array())
                {
                   ...
                   $table.="<tr>";
                    ...
                    if($fetch_All[4]!=""){
                        $table.="<td>"."<a href=\"../other_img/".$fetch_All[0].".".$fetch_All[4]."\" target='_blank' class='btn btn-primary'>Download</a>"."</td>";
                    }
                    else
                    {
                        $table.="<td>"."<span><strong class='text-danger'>No File Found</strong></span>"."</td>";
                    }
                        ...
                        $table.="<a style='width:80px; margin-left:5px; margin-top:2px;' href='?dlt=$fetch_All[0]&ext=$fetch_All[4]' class='btn btn-danger' onclick='return confirm_delete()'>Delete</a>";
                        $table.="</td>";
                 $table.="</tr>";
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `public_exam_result` WHERE `serial`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='public_exam_result.php'</script>";
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM public_exam_result WHERE `serial`='".$_GET["edit"]."'";
        ...
        $expensions= array("jpeg","jpg","png","pdf","","txt");         
                ...
                if(in_array($file_ext,$expensions) === false){
                 ...
                 $msg='File size must be excately 2 MB';
                    ...
                    //print "asdf";
                @$inserquery="Replace INTO `public_exam_result` (`serial`,`date`,`result_title`,`result`,`extension`) VALUES('".$_POST["id"]."','$date','$title','$detaile','$file_ext')";
                   ...
                   // print "asdfasdf";
                                    ...
                                    @$inserquery="Replace INTO `public_exam_result` (`serial`,`date`,`result_title`,`result`) VALUES('".$_POST["id"]."','$date','$title','$detaile')";
...
<html lang="en">
  <head>
    ...
    <?php if(isset($_POST["view"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Record  Found"."<a href='public_exam_result.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
...
Results of public examinations</span> </td>
            </tr>
         <tr class="info">
        ...
        <td>
          ...
          <input type="text"   id="example1" name="date" <?php if($chek_query){?> value="<?php echo $fetch_query[1];?>" <?php } else {?> value="" <?php }?> class="form-control" placeholder="Date" />
          </div>
          </td>
      </tr>
      ...
      <tr class="info">
        ...
        <td>
          <div class="col-sm-12 col-md-12">
          <input type="hidden" name="id"  <?php if($chek_query){?>  value="<?php echo $fetch_query[0];?>" <?php } else {?> value="" <?php  }?></input>
...
Exam Result&nbsp;</td>
        <td>:</td>
         <td>
            <div class="col-sm-12 col-md-12">
            <textarea  name="descreption"  disabled="disabled" class="form-control"  id="redactor"><?php if($chek_query){echo $fetch_query[3]; } else { echo ""; }?> </textarea>
            </div>
            </td>
    ...
    <tr class="info">
            ...
            <td>:</td>
            <td><input type="file"  disabled="disabled" name="file" accept="image/*" />
