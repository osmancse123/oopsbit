   ...
   global $checked_query;
    ...
    if(isset($_POST['add']))
    {
      ...
      $about_school=$db->escape($_POST['Contact_Textarea']);
    	...
    	{
    	$insert="INSERT INTO `activities` VALUES('1','$title','$about_school')";
    	$db->insert_query($insert);$select="SELECT * FROM `activities`";
    $checked_query=$db->select_query($select);
    if($checked_query)
    {
      $fetch_zero=$checked_query->fetch_array();
  ...
  if(isset($_POST['Update']))
    {
      ...
      {
        $insert="REPLACE INTO `activities` VALUES('1','$title','$about_school')";
      	$db->update_query($insert);$select="SELECT * FROM `activities`";
    $checked_query=$db->select_query($select);
    if($checked_query)
    {
      $fetch_zero=$checked_query->fetch_array();
...
if(isset($_POST['Delete'])){
	$Delete="DELETE FROM `activities` WHERE `id`='1'";
	$db->delete_query($Delete);
  print "<script>location='activities.php'</script>";
