<?php
    ...
    global $chek_query;
	$prefix=date("y"."m"."d");
$fetch_query[0]=$db->withoutPrefix('defferentexam','defferentxm_id',"15".$prefix,'12');
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="INSERT INTO `defferentexam`(`defferentxm_id`,`date_time`,`xm_title`,`description`,`extension`) VALUES('".$fetch_query[0]."','$date','$title','$detaile','$file_ext')";
...
if(isset($_POST["view"]))
{
    $query="select * from defferentexam ORDER BY `Date_time` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='defferenexam.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>বিভিন্ন পরীক্ষার ফলাফলের তথ্য সমূহ</span>"."</td>"."</tr>";
        if($result){
            ...
            $count=mysqli_num_fields($result);
                while($fetch_All=$result->fetch_array())
                {
                   $table.="<tr>";
                    ...
                    {$table.="<td>".'<a href="../other_img/'.$fetch_All[0].'.'.$fetch_All[4].'" class="btn btn-primary" target="_blank">Download</a>'."</td>";}
                    else
                    {
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `defferentexam` WHERE `defferentxm_id`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='defferenexam.php'</script>";
   $fetch_query[0]=$db->withoutPrefix('defferentexam','defferentxm_id',"15".$prefix,'12');
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM `defferentexam` WHERE `defferentxm_id`='".$_GET["edit"]."'";
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="REPLACE INTO  `defferentexam`(`defferentxm_id`,`date_time`,`xm_title`,`description`,`extension`) VALUES('".$_POST["id"]."','$date','$title','$detaile','$file_ext')";
...
<html lang="en">
 ...
 function checkforText()
    ...
    <?php if(isset($_POST["view"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Recorde  Found"."<a href='defferenexam.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
...
বিভিন্ন পরীক্ষার ফলাফলের তথ্য  </span> </td>
            </tr>
         <tr class="info">
        <td align="right">তারিখ &nbsp;</td>
        <td>:</td>
        <td>
      ...
      <tr class="info">
        <td align="right">পরীক্ষা  টাইটেল &nbsp;</td>
        <td>:</td>
        <td>
          <div class="col-sm-12 col-md-12">
...
বিস্তারিত লিখুন &nbsp;</td>
        <td>:</td>
         <td>
            <div class="col-sm-12 col-md-12">
            <textarea  name="descreption" class="form-control" disabled="disabled"  id="redactor"><?php if($chek_query){echo $fetch_query[3]; } else { echo ""; }?> </textarea>
            </div>
            </td>

    </tr>
    <tr class="info">
            <td align="right" width="180">  ফাইল আপলোড &nbsp; </td>
            <td>:</td>
