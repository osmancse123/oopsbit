...
function showChekTo(){
	...
	function deleteBYid(stid){
			...
			var clsid = $('#clsid-'+stid).val();
			var gpid = $('#gpid-'+stid).val();
			var ExamId = $('#ExamId-'+stid).val();
			var SubjectId = $('#SubjectId-'+stid).val();
			var SubjectPartID = $('#SubjectPartID-'+stid).val();
			var Session = $('#Session-'+stid).val();
			var gpid = $('#gpid-'+stid).val();
			var deltesub = "delsub";
		
		
			$.ajax({
				url:"deletsubjectmark.php",
				type:"POST",
				data :{gpid:gpid,ExamId:ExamId,SubjectId:SubjectId,SubjectPartID:SubjectPartID,Session:Session,stid:stid,deltesub:deltesub,clsid:clsid},
				cache:false,
				success:function(data)
				{
					//alert(data);
