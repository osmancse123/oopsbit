				...
				if(!empty($_POST["date"]) && !empty($_POST["stdid"])){
					...
					$resultID=$db->select_query($chekId);
						if($resultID->num_rows > 0){
									$inserSql="REPLACE INTO `recordforcertificate` (`Date`,`StudentID`) VALUES ('".$_POST["date"]."','".$_POST["stdid"]."')"; 
									$resultsql=$db->update_query($inserSql);
									print "<script>location='viewCertificate.php?date=$_POST[date]&stdid=$_POST[stdid]'</script>";
						}
						else {
    ...
    <form name="notice" action="" method="post"  enctype="multipart/form-data" class="form-horizontal" >
   				<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 col-lg-offset-2 col-md-offset-2">
						<table align="center" class="table table-responsive" style="border:1px #CCCCCC solid; margin-top:30px; color: #000;">
								   <tr>
									<td  class="warning" colspan="3" align="center"><span style="font-size:22px; color:#333; display:block;">Character Certificate</span> </td>
								</tr>
								 <tr>
									<td>Date</td>
									<td>:</td>
