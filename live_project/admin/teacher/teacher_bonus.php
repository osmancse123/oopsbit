...
function ADDandDelete(){
	...
	if(paidAmount != '' && teacherId !='' ){
	 $.ajax({
                    ...
                    type: "POST",
                    url: "ajaxForBonus.php",
                    data:$(".dataFrom").serialize() + "&SaveData=" + AddandDelete,
                    cache: false,
...
function ForUpdate(id)
{
 ...
 $.post("ajaxForBonus.php", { Mid: Mid,update:update,amount:amount },
            function(result){
...
function Fordelete(id)
{
 ...
 $.post("ajaxForBonus.php", { Mid: Mid,deleteS:deleteS },
            function(result){
  ...
  <body>
    <form name="" action="#" method="post"  enctype="multipart/form-data" class="form-horizontal addData dataFrom">
      <div class="has-feedback col-xs-12 col-sm-8 col-lg-10 col-md-10  col-md-offset-1" id="frstPage">
        ...
        <tr>
  				<td bgcolor="#f4f4f4"  class="warning" colspan="3" bgcolor="#dddddd" align="center"><span style="font-size:22px; color:#333; display:block;">Teacher Bonus Setting</span> </td>
  			</tr>
...
<div class="col-md-12"> <input type="text" autocomplete="off" name="teacherId"  onKeyUp="return showIdby()"  id="teacherId" placeholder='Teacher ID' style="width:178px;  height:30px;"/> Month : <select class="" style="width:178px;  height:30px;" name="month">
...
</select> Year : <input type="text" name="year" value="<?php echo date("Y") ?>">
...
<tr>
	...
	<td>:</td>
	<td>
		<select style="width:60%;  height:30px; padding-left:5px;" name="bonus_type">
			<option selected="" disabled="">Select One</option>
			<?php
$sql="select * from add_payment_title";
$quer=$db->select_query($sql);
if ($quer) {
	while ($fetch=mysqli_fetch_assoc($quer)) {
		?>
		<option value="<?php echo $fetch["id"]?>"><?php echo $fetch["payment_title"];?></option>
		<?php
...
<tr>
	<td>Amount</td>
	<td>:</td>
	<td><input type="text" name="amount" style="height: 30px"> </td>
</tr>
