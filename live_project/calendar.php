...
<div class="col-md-9 col-xs-12 fontsize backgroundcol"  style="padding:0px ; margin:0px; margin-top:10px; padding-top:10px;" >
				...
				padding:0px; float:left; clear:right; text-align:left;">
						<span id="noticetitle"  style="color:#000000; font-size:20px; font-family:'Times New Roman', Times, serif; font-weight:300;">Academic Calendar</span>
				</div>
		...
		<div class="col-md-12 col-xs-12" style="margin:0px; padding:0px; margin-top:5px;">
				<table class="table table-bordered table-responsive">
						...
						<?php
						$sql="SELECT COUNT(ID) FROM `academic_calender`";
	...
	$sql1= "SELECT * FROM `academic_calender` ORDER BY ID DESC $limit";
	...
	if($last != 1){
		...
		{
			$previous = $pagenum-1;
			$pagenationCtrl.= '<a href="'.$_SERVER['PHP_SELF'].'?page=calendar&pn='.$previous.'" >Previous</a> &nbsp;';
				for($i = $pagenum-4;$i < $pagenum; $i++){
					
					if($i > 0){
						
						$pagenationCtrl.= '<a href="'.$_SERVER['PHP_SELF'].'?page=calendar&pn='.$i.'" >'.$i.'</a> &nbsp;';
					}
			...
			for($i = $pagenum+1;$i <= $last; $i++){
				...
				{
					$next=$pagenum+1;
					$pagenationCtrl.='&nbsp; &nbsp;<a href="'.$_SERVER['PHP_SELF'].'?page=calendar&pn='.$next.'" >Next</a>';
				}
	}				
					...
					$sl++;
						?>
								 <tr>
									<td align="center"><span ><?php echo $sl;?></span></td>
									<td><a class="changetitle" href="?page=noticeview&calid=<?php echo $fetch->ID;?>" class="changetitle" style="text-decoration:none; color:#000000; font-family:'Times New Roman', Times, serif" id="titlecolor"><?php echo $fetch->Title;?></a></td>
