...
<div class="col-md-9 col-xs-12 fontsize backgroundcol"  style="padding:0px ; margin:0px; margin-top:10px; padding-top:10px;" >
				...
				padding:0px; float:left; clear:right; text-align:left;">
						<span id="noticetitle"  style="color:#000000; font-size:20px; font-family:'Times New Roman', Times, serif; font-weight:300;">Various Exam Rules </span>
				</div>
		...
		<div class="col-md-12 col-xs-12" style="margin:0px; padding:0px; margin-top:5px;">
				<table class="table table-bordered table-responsive">
						...
						<?php
						$sql="SELECT COUNT(defferentxm_id) FROM `defferentexam`";
	...
	$sql1= "SELECT * FROM `defferentexam` ORDER BY defferentxm_id DESC $limit";
	...
	if($last != 1){
		...
		{
			$previous = $pagenum-1;
			$pagenationCtrl.= '<a href="'.$_SERVER['PHP_SELF'].'?page=various&pn='.$previous.'" >Previous</a> &nbsp;';
				for($i = $pagenum-4;$i < $pagenum; $i++){
					
					if($i > 0){
						
						$pagenationCtrl.= '<a href="'.$_SERVER['PHP_SELF'].'?page=various&pn='.$i.'" >'.$i.'</a> &nbsp;';
					}
			...
			for($i = $pagenum+1;$i <= $last; $i++){
				...
				{
					$next=$pagenum+1;
					$pagenationCtrl.='&nbsp; &nbsp;<a href="'.$_SERVER['PHP_SELF'].'?page=various&pn='.$next.'" >Next</a>';
				}
	}				
					...
					$an=preg_replace('#[^a-z,A-Z,অ-ঔ,ক-ঁ,া,ি,ী,ু,ূ,ে,ৈ,ো,ৌ,০-৯,0-9]#','',$fetch->xm_title);
					$sl++;
						?>
								 <tr>
									<td align="center"><?php echo $sl;?></td>
									<td><a class="changetitle" href="?page=noticeview&various=<?php echo $fetch->defferentxm_id;?>" style="text-decoration:none; color:#000000; font-family:'Times New Roman', Times, serif" id="titlecolor"><?php echo $fetch->xm_title;?></a></td>
