...
<div class="col-md-9 col-xs-12 fontsize backgroundcol"  style="padding:0px ; margin:0px; margin-top:10px; padding-top:10px;" >
				...
				padding:0px; float:left; clear:right; text-align:left;">
						<span id="noticetitle"  style="color:#000000; font-size:20px; font-family:'Times New Roman', Times, serif; font-weight:300;"><?php echo strtoupper($_GET["page"]."   Based Routine" )?></span>
				</div>
		...
		<div class="col-md-12 col-xs-12" style="margin:0px; padding:0px; margin-top:5px;">
				<table class="table table-bordered table-responsive">
						...
						<?php
							$sql="SELECT COUNT(`Routine_id`) FROM `routine` where `Routine_type`='".$_GET["page"]."'";
	...
	$sql1= "SELECT * FROM `routine`  where `Routine_type`='".$_GET["page"]."'  ORDER BY Routine_id DESC $limit";
		...
		if($pagenum > 1 )
		{
			...
			for($i = $pagenum+1;$i <= $last; $i++){
				...
				if($pagenum != $last)
				{
					...
					$an=preg_replace('#[^a-z,A-Z,অ-ঔ,ক-ঁ,া,ি,ী,ু,ূ,ে,ৈ,ো,ৌ,০-৯,0-9]#','',$fetch->Routine_title);
					$sl++;
						?>
								 <tr>
									<td align="center"><?php echo $sl;?></td>
									<td><a class="changetitle" href="?page=noticeview&routineid=<?php echo $fetch->Routine_id;?>" style="text-decoration:none; color:#000000; font-family:'Times New Roman', Times, serif" id="titlecolor"><?php echo $fetch->Routine_title;?></a></td>
